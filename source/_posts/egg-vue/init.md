...
- `easywebpack-cli` 通过项目根目录下的 `webpack.config.js` 配置文件构造出 Webpack 实际的配置文件，配置项请见 [webpack.config.js](/easywebpack/config)
...
在app/web/page 目录下面创建home目录, home.vue 文件, Webpack自动根据.vue文件创建entry入口, 具体实现请见[webpack.config.js](/easywebpack/config)
...
```html
<template>
  <layout title="egg vue ssr" description="vue server side render"
